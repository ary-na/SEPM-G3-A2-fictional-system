    ...
    public void displayTickets() {
        ...
        for (Ticket item : tickets) {
            if (item.getStatus().equals(Status.OPEN)) {
                System.out.println(item.getId() + " Description: " + item.getDescription() + " Severity: " + item.getSeverity() + " Status: " + item.getStatus() + " Technician Assigned: " + item.getAssignedTo().get(item.getAssignedTo().size() - 1).getFullName());
                System.out.println("---------------------------------------------------------------------------------");
            }
        }
    }
    ...
    public void displayAllTickets() {
        ...
        for (Ticket ticket : tickets) {
            System.out.println("[" + ticket.getId() + "] Staff Full Name: " + this.getFullName() + " Description: "
                    + ticket.getDescription() + " Severity: " + ticket.getSeverity() + " Status: " + ticket.getStatus() + " Technician Assigned: " + ticket.getAssignedTo().get(ticket.getAssignedTo().size() - 1).getFullName());
